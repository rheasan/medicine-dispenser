...
const FirestoreHandler = () => {
    ...
    const getAllDocs = () =>{
        ...
        .then((medicineData)=>{
            return medicineData.map((obj)=>{
                return {name: obj.name, time: new Date(obj.time), timeString: obj.timeString};
            })
        })
        .catch((err)=>{throw err});
        return medicineData;
    ...
    const addDoc = async (medicine) => {
        ...
        .set({name: medicine.name, time: medicine.time.toString(), timeString: _getTime(medicine.time)});
    }
