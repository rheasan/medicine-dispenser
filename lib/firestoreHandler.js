...
const FirestoreHandler = () => {
    ...
    const getAllDocs = () =>{
        ...
        .then((medicineData)=>{
            return medicineData.map((obj)=>{
                return {name: obj.name, time: new Date(obj.time), hours: obj.hours, minutes: obj.minutes};
            })
        })
        .catch((err)=>{throw err});
        return medicineData;
    ...
    const addDoc = async (medicine) => {
        ...
        .set({name: medicine.name, time: medicine.time.toString(), hours: medicine.time.getHours(), minutes: medicine.time.getMinutes()});
    }
